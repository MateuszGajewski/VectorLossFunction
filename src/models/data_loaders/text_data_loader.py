...
class TextDataLoader:
    ...
    def get_data_loaders(self, config):
        ...
        self.vocab = build_vocab_from_iterator(yield_tokens(dataset_train), specials=["<unk>"])
        self.vocab.set_default_index(self.vocab["<unk>"])

        train_loader = DataLoader(dataset_train, batch_size=int(config['training']['batch_size']),
                                       shuffle=True, pin_memory=True, collate_fn=self.collate_batch)
        test_loader = DataLoader(dataset_test, batch_size=int(config['training']['batch_size']),
                                      shuffle=True, pin_memory=True, collate_fn=self.collate_batch)
        cls = eval(config['training']['classifier'])(len(self.vocab), int(config['data']['embed_dim']),
