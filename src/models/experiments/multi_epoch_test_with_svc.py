...
def run_stability_test(
    server_uri, top_n_models, experiment_name, by_parameter, file, epochs
):
    runs_params = []
    parameters = [
        "params.Batch size",
    ...
    exp = exp[exp["params.number of dimensions"] != 0]

    for b in batch_sizes:
        tmp_exp = exp[exp[by_parameter] == b]
        tmp_exp = tmp_exp.sort_values(
...
def set_value_in_property_file(
    file_path, batch_size, recalculate_period, out_dim, learning_rate, epochs
):
...
if __name__ == "__main__":
    run_stability_test(
        "http://127.0.0.1:5000",
        3,
        "grid_search_one_epoch",
        "params.Batch size",
        Path("../configs/visual_config_davis_bouldin_multi_epoch_svc.ini"),
        10,
    )
