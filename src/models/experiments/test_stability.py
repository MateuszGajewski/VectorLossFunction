...
def run_stability_test(server_uri, top_n_models, experiment_name, by_parameter, file, repeat):
    runs_params = []
    ...
    for i in runs_params:
        set_value_in_property_file(file, i['params.Batch size'], i['params.recalculate_period'],
                                   i['params.number of dimensions'], i['params.learning_rate'])
        for j in range(0, repeat):
            experiment = Experiment(file, name=str(i))
...
if __name__ == "__main__":
    run_stability_test("http://127.0.0.1:5000", 5, 'grid_search_one_epoch', 'params.Batch size',
                       Path("../configs/visual_config_davis_bouldin_stability_test_one_epoch.ini"),
                       10)