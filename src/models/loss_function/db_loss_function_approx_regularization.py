...
class DBLossFunctionApproxRegularization(DBLossFunctionApprox):
    def __init__(self, device='cpu', json=None, approx_size=0.05):
        super(DBLossFunctionApproxRegularization, self).__init__(device, json, approx_size)
    ...
    def calculate_loss(self, predicted, target):
        ...
        regularization = self.calculate_centroids_update(predicted, target, self.centroids.detach().clone().to(self.device))
