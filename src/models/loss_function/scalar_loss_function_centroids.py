...
class ScalarLossFunctionCentroids(AbstractLossFunction):

    def __init__(self, device='cpu', json=None):
        super(ScalarLossFunctionCentroids, self).__init__()
        self.device = device
    ...
    def calculate_loss(self, predicted, target):
        ...
        for c in range(0, self.class_number):
            ...
            if idx.shape[0] > 0:
                ...
                alphas +=(idx.shape[0]/target.shape[0])*\
                         (1+(torch.mm(examples_normalized, centroids[c].T).sum())) \
                          / 2*((examples_normalized.shape[0]))

                betas += (1 + torch.mm(examples_neg_normalized, examples_normalized.T).sum()) \
                          / 2* (examples_normalized.shape[0] * examples_neg_normalized.shape[0])
