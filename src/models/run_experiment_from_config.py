...
class Experiment:
    ...
    def build_objects(self):
        ...
        if self.config.has_option("data", "labels_hierarchy"):
            self.label_hierarchy = json.load(
                open(self.config["data"]["labels_hierarchy"])
            )

        if self.config.has_option("training", "validate_after_epoch"):
            self.validate_after_epoch = self.config.getboolean(
                "training", "validate_after_epoch"
            )

        if (
            self.config.has_option("data", "labels_hierarchy")
            and (self.config["training"]["loss_function"]).split(".")[0]
            == "loss_function"
        ):
            ...
            self.criterion = eval(self.config["training"]["loss_function"])(
                self.device, self.label_hierarchy
            )
        ...
        if self.config.has_option("inference", "vector_to_label_transformer"):
            self.vector_to_label_transformer = eval(
    ...
    def train_and_log_model(self):
        ...
        if self.vector_to_label_transformer:
            mlflow.log_param(
                "vector_to_label_transformer",
                self.config["inference"]["vector_to_label_transformer"],
            )
        if self.config.has_option("training", "recalculate_period"):
            mlflow.log_param(
                "recalculate_period", self.config["training"]["recalculate_period"]
            )
        self.train()
        self.validate()
...
if __name__ == "__main__":
    ...
    experiment = Experiment(Path("./configs/ag_news_config_davies_bouldini.ini"))
